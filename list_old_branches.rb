...
# = develop/masterにマージされないまま10日間以上たったbranchをリスト化
module ListOldBranches
  ...
  def self.execute
    ...
    @repos.each do |repo|
      ...
      # find f branches that not yet merged to develop
      not_mergeds = `cd #{d} && git branch -r --no-merged origin/develop`.
        split("\n").
        grep(%r{^\s*origin/[hf]/}).map {|b|
        remotes.detect{|r| r.name == b.sub(%r{^\s*origin/}, "")}
      }
      # find h branches that not yet merged to master
      not_mergeds.concat `cd #{d} && git branch -r --no-merged origin/master`.
        split("\n").
        grep(%r{^\s*origin/h/}).map {|b|
        remotes.detect{|r| r.name == b.sub(%r{^\s*origin/}, "")}
      }
